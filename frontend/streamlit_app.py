...
st.caption("Phase 7: morning notes over what changed, plus prices, financials, comps, "
           "pipeline, LOE, approvals, catalysts, and news.")
...
with whatchanged_tab:
    ...
    note_ticker = note_cols[0].selectbox("Company", tickers, index=default_index,
                                         key="note_ticker")
    if note_cols[1].button("Generate note", key="gen_note"):
        st.session_state["note"] = api_get(
            api_base, f"/companies/{note_ticker}/note?refresh=true")
    elif st.session_state.get("note", {}).get("ticker") != note_ticker:
        st.session_state["note"] = api_get(api_base, f"/companies/{note_ticker}/note")

    note = st.session_state.get("note") or {}
    if not note.get("body"):
        st.info(f"No note for {note_ticker} yet. Press generate.")
    else:
        st.markdown(note["body"])
        layer = ("rules layer, no Anthropic key set"
                 if note.get("model") == "rules" else f"generated by {note.get('model')}")
        st.caption(f"{layer}. Generated {note.get('generated_at')}.")
    if note.get("error"):
    ...
    if not feed:
        ...
        st.caption(
            ...
            "near-term LOE, ranked by significance. The note above summarises one "
            "company's slice of this feed."
        )
