...
# Ten years of daily closes, about 2,500 sessions per company, so the chart reaches back
# past 2020. The chart cannot show history that was never fetched. Cost is one request
# either way, since Yahoo returns the whole range in a single response, and the upsert is
# keyed on (company_id, as_of) so a wider pull backfills rather than duplicates.
CHART_RANGE = "10y"
CHART_INTERVAL = "1d"
