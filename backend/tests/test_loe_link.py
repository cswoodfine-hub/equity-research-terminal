...
def test_an_elapsed_patent_term_is_an_loe_in_the_past(tmp_path, monkeypatch):
    ...
    assert got["past"] is True and got["date"] == "2014-12-31"
    assert got["basis"].startswith("statutory patent term elapsed")
...
def test_sync_writes_an_approval_once_from_openfda(tmp_path, monkeypatch):
    ...
    assert conn.execute("SELECT modality FROM assets WHERE id = 12").fetchone()[0] \
        == "small molecule"
